package com.basics.demo;

import java.time.Duration;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;

import io.github.bonigarcia.wdm.WebDriverManager;

public class DynamicWebTableWithPagination {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		WebDriverManager.chromedriver().setup();
		WebDriver driver = new ChromeDriver();
		
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
		driver.manage().window().maximize();
		
		driver.get("https://demo.opencart.com/admin/index.php");
		
		WebElement userName = driver.findElement(By.id("input-username"));
		userName.clear();
		userName.sendKeys("demo");
		
		WebElement  pass = driver.findElement(By.id("input-password"));
		pass.clear();
		pass.sendKeys("demo");
		
		driver.findElement(By.xpath("//button[@class='btn btn-primary']")).click();
		
		driver.findElement(By.xpath("//a[@href='#collapse26']")).click();
		driver.findElement(By.xpath("//ul[@id='collapse26']//a[text()='Orders']")).click();
		
		//1) Total number of pages 
		String text =driver.findElement(By.xpath("//div[@class='col-sm-6 text-right']")).getText();
		System.out.println(text);

		String pageText = text.substring(text.indexOf("(")+1, text.indexOf("Pages")-1);
		System.out.println(pageText);
		int totalPages  = Integer.valueOf(pageText);
		
		//2)get total number of rows in each page  and navigate to all pages 
		for(int p=1; p<=1; p++) {
			WebElement activePage = driver.findElement(By.xpath("//ul[@class='pagination']//span"));
			
			System.out.println("Current Active page "+activePage.getText());
			
			int totalrows = driver.findElements(By.xpath("//table[@class='table table-bordered table-hover']//tbody/tr")).size();
			System.out.println("total rows :"+totalrows+" " );
			
			//Get all rows data before going to next page
			int totalcols =driver.findElements(By.xpath("//table[@class='table table-bordered table-hover']//thead/tr/td")).size();

			//to print all data in rows
			/*for(int r=1;r<=totalrows; r++) {
				for(int c=1;c<=totalcols; c++) {
					String data = driver.findElement(By.xpath("//table[@class='table table-bordered table-hover']//tbody/tr["+r+"]/td["+c+"]")).getText();
					System.out.println(" "+data+" ");
				}
			} */
			
			//to print specific rows, based on Status=pending
			for(int r=1;r<=totalrows; r++) {
				String ordeId = driver.findElement(By.xpath("//table[@class='table table-bordered table-hover']//tbody/tr["+r+"]/td[2]")).getText();
				String customer = driver.findElement(By.xpath("//table[@class='table table-bordered table-hover']//tbody/tr["+r+"]/td[3]")).getText();
				String status = driver.findElement(By.xpath("//table[@class='table table-bordered table-hover']//tbody/tr["+r+"]/td[4]")).getText();

					if(status.equals("Pending")) {
						System.out.println(" "+ordeId+" "+customer+" "+status);
					}
				}
			
			//next page click
			String page = Integer.toString(p+1);
			driver.findElement(By.xpath("//ul[@class='pagination']//li/a[text()='"+page+"']")).click();
			
			
		}
		
		
	}

}
